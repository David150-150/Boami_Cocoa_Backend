...
async def combined_diagnosis(
    ...
    try:
        ...
        # =========================================================
        # 2. IMAGE DATA (FROM DB)
        # =========================================================
        image_confidence = scan.confidence_score or 0.0
        image_disease = scan.disease.name if scan.disease else None

        # =========================================================
        # 3. SYMPTOM / VOICE DATA (MISSING IN YOUR DB CURRENTLY)
        #    TEMP SAFE FALLBACK (until you store properly)
        # =========================================================
        symptom_confidence = getattr(scan, "symptom_confidence", 0.0)
        symptom_disease = getattr(scan, "symptom_disease", None)

        # =========================================================
        # 4. SAME DISEASE CHECK
        # =========================================================
        if image_disease and symptom_disease:
            same_disease = (
                normalize_label(image_disease) ==
                normalize_label(symptom_disease)
            )
        else:
            same_disease = False
        ...
        if same_disease:
            final_confidence = min(
                ...
                (symptom_confidence * SYMPTOM_WEIGHT) + 0.05
            )
            final_disease = image_disease
        else:
            final_confidence = round(image_confidence * IMAGE_WEIGHT, 4)
            final_disease = image_disease or symptom_disease

        # =========================================================
        # 6. URGENCY (CORRECT VERSION)
        # =========================================================
        urgency = get_urgency(final_confidence, final_disease)
        ...
        update_scan(
            ...
            payload.scan_id,
            {
                "confidence_score": final_confidence,
                "urgency_level": urgency,
                "description": f"Combined AI result: {final_disease or 'Healthy'}",
            },
        )
