...
class ImagePredictor:
    ...
    MODEL_PATH           = "app/_ai/model/best.pt"
    CONFIDENCE_THRESHOLD = 0.25   # minimum raw YOLO confidence
    HEALTHY_MIN_CONF     = 0.75   # must be this confident to call it "Healthy"
    ...
    def predict(self, image_bytes: bytes) -> dict:
        ...
        if result.boxes is None or len(result.boxes) == 0:
            return {
                ...
                "confidence_percent": "0.00%",
                "annotated_image":    self._encode(img),
                "message":            "No disease detected. Please retake in good lighting.",
            }
        ...
        # --- Healthy result ---
        if label.lower() == "healthy":
            if conf < self.HEALTHY_MIN_CONF:
                # Not confident enough — tell farmer to use voice/symptoms
                return {
                    "status":             "Uncertain",
                    "disease":            None,
                    "confidence":         conf,
                    ...
                    "annotated_image":    annotated_bytes,
                    "message":            (
                        "Could not clearly identify the pod condition. "
                        "Please retake the photo in good natural lighting "
                        "or describe symptoms using voice or symptom form."
                    ),
                }
            return {
                "status":             "Healthy",
    ...
    def get_model_info(self) -> dict:
        return {
            ...
            "threshold":        self.CONFIDENCE_THRESHOLD,
            "healthy_min_conf": self.HEALTHY_MIN_CONF,
        }